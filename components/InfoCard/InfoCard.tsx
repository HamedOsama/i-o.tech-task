...
const InfoCard = ({ title, value, Icon, bg, link }: IInfoCardProp) => {
  ...
  return (
    <div className='bg-white rounded-md'>
      ...
      <div className={`flex items-center justify-between px-2 py-3 cursor-pointer ${bg} hover:bg-opacity-75 duration-200 group`} onClick={handleClick}>
        <p className='text-white'>See Details</p>
        <ArrowRightAltRoundedIcon className='text-white group-hover:animate-pulse' />
      </div>
    </div>
  )
}
